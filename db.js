...
// resultSide, timestamp}) - the "who's the opponent / did I win" transform
// happens here so the client doesn't need any new logic, just a place to
// drop the array in.
async function loadCoinflipHistoryForUser(username, limit) {
  const res = await pool.query(
    `SELECT creator, joiner, creator_items, joiner_items, creator_value, joiner_value, result, winner, resolved_at
