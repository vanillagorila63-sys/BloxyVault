...
function syncAccount(username) {
  ...
  const u = ensureUser(username);
  send(ws, { type: 'account', user: { username, coins: u.coins, inventory: u.inventory, stats: u.stats, specialRole: u.specialRole || null, mutedUntil: u.mutedUntil || 0 } });
}
...
function handleChatSend(username, msg, ws) {
  ...
  // broadcast as text even if a non-mod types them (the permission check
  // below just quietly rejects it with a private error, same as if they'd
  // tried any other admin action they don't have).
...
wss.on('connection', (ws) => {
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'login') {
      ...
      const u = ensureUser(username);
      send(ws, { type: 'login:ok', user: { username, coins: u.coins, inventory: u.inventory, stats: u.stats, specialRole: u.specialRole || null, mutedUntil: u.mutedUntil || 0 } });
