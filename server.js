...
function ensureUser(username) {
  if (!users.has(username)) {
    if (username === GIVEAWAY_USERNAME) {
      // Stock the giveaway account with 99 of every pet in the catalog
      // (same "99x" display convention as the coin shop) instead of the
      // usual empty inventory, plus a large coin balance - this account
      // exists to be tipped out from, not to play with. Tips from this
      // account don't actually decrement stock either (see handleTipSend)
      // so it always reads 99x no matter how much gets given away.
      const inventory = {};
      for (const id of Object.keys(petCatalog)) inventory[id] = 99;
      users.set(username, { coins: GIVEAWAY_STARTING_BALANCE, inventory, stats: { wagered: 0, won: 0, lost: 0 } });
    } else {
