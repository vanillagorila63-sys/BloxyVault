...
function ensureUser(username) {
  ...
  if (isNewAccount) {
    ...
    users.set(username, { coins: startingCoins, inventory: startingInventory, stats: { wagered: 0, won: 0, lost: 0 }, specialRole: null });
    if (isGiveaway) {
...
function syncAccount(username) {
  ...
  const u = ensureUser(username);
  send(ws, { type: 'account', user: { username, coins: u.coins, inventory: u.inventory, stats: u.stats, specialRole: u.specialRole || null } });
}
...
wss.on('connection', (ws) => {
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'login') {
      ...
      const u = ensureUser(username);
      send(ws, { type: 'login:ok', user: { username, coins: u.coins, inventory: u.inventory, stats: u.stats, specialRole: u.specialRole || null } });
