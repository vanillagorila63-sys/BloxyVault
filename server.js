...
function resolveJackpot() {
  ...
  if (round.entrants.length < JACKPOT_MIN_ENTRANTS) {
    // Not enough players yet - a Jackpot round should never start/resolve
    // with just one participant. Keep the round open and give it another
    // full window instead of paying out (or discarding) a lone entrant.
    round.endsAt = Date.now() + JACKPOT_ROUND_MS;
    round.timer = setTimeout(resolveJackpot, JACKPOT_ROUND_MS);
    broadcastJackpot();
    return;
  }
  jackpotRound = null;
...
async function runBattle(b) {
  ...
  let winningGroup, jackpotInfo = null;
  if (winMode === 'crazy') {
    // Crazy inverts the normal rule: lowest total value wins.
    winningGroup = [...groups].sort((a, c) => a.total - c.total)[0];
  } else if (winMode === 'terminal') {
    // Terminal ignores the running total entirely - only the single biggest
    // pull from the final case/round decides it.
    winningGroup = [...groups].sort((a, c) => c.lastPull - a.lastPull)[0];
    ...
    winningGroup = groups[groups.length - 1];
    let cum = 0;
    ...
    // Normal: highest total value wins, same rule as before this update.
    winningGroup = [...groups].sort((a, c) => c.total - a.total)[0];
  }

  const winner = winningGroup.label;
  const winnerValue = battleTotal; // winning side takes the full combined pot, same as before
  const share = Math.floor(battleTotal / Math.max(1, winningGroup.members.length));

  winningGroup.members.filter((p) => !p.isBot).forEach((p) => {
  ...
  b.players.filter((p) => !winningGroup.members.includes(p) && !p.isBot).forEach((p) => {
...
function handleCaseOpen(username, msg) {
  ...
  u.coins += wonPet.value; // pulls convert straight to coins, matching the existing "no inventory needed" case design
  trackWager(username, c.price);
  persistUser(username, { type: 'case_open', amount: wonPet.value - c.price, balanceBefore: before, balanceAfter: u.coins });
  syncAccount(username);
  send(errTo, {
    type: 'case:opened',
    caseIndex,
    ...
    pool: pool.map((e) => ({ id: e.id, chance: e.chance })),
  });
  pushFeedEvent({ game: 'cases', username, amount: wonPet.value, itemName: wonPet.name });
}
